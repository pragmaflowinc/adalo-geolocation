...
export function GeoFilteredListComponent(props: GeoFilteredListProps) {
  ...
  if (
    ...
    listProps.items = listProps.items.reduce((acc, item) => {
      if (item.secondLine) {
        ...
        item.secondLine.text = `${distance}${
          props.unitsOfMeasure === "imperial" ? "mi" : "km"
        }${
          item.secondLine && item.secondLine.text
            ? ` - ${item.secondLine.text}`
            : ""
        }`;
  ...
  if (listProps.listEmptyState && listProps.items && !listProps.items[0]) {
    return <EmptyState {...listProps.listEmptyState!}></EmptyState>;
  } else {
    // @ts-ignore
    return <SimpleList {...listProps} />;
  }
}
