  ...
  if (listProps.items && listProps.items.length > 0 && listProps.items[0].secondLine?.displayDistance) {
    listProps.items = listProps.items.reduce((acc, item) => {
      if (item.secondLine) {
        ...
        item.secondLine.text = `${distance / (props.unitsOfMeasure === 'imperial' ? METER_TO_MILE : METER_TO_KILOMETER)}${props.unitsOfMeasure === 'imperial' ? 'mi' : 'km'}${item.secondLine && item.secondLine.text ? ` - ${item.secondLine.text}` : ''}`
